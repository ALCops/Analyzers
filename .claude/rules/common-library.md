...
- `Permissions/` — Shared permission model for AC0031 (missing) and AC0032 (unused): `DatabaseOperation`, `RequiredPermissionDetector`, `PermissionResolver`. Two disjoint method maps on purpose: `MethodOperationMap` is keyed on built-in methods called *on a record receiver* (and is mirrored by `RecordMethodClassification`), while `DataTransferOperations` covers the `DataTransfer` executors, whose tables come from `SetTables` arguments instead of the receiver — merging them would make any user-defined `CopyFields`/`CopyRows` look like a DB operation. `RequiredPermissionDetector.TryGetFromDataTransfer` is the only entry point that can report *unresolvable* (returns `false`); both cops must then treat the access as targeting an unknown table.
