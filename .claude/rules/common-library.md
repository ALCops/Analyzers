...
- `FlowTerminatingBuiltIns.cs` — Shared semantic classifier for `Dialog.Error`, `Table.FieldError`, and `FieldRef.FieldError`. Clean bindings require the exact built-in class/method pair; invalid editor-time bindings use the corresponding `NavTypeKind`. It deliberately treats collectible `Error(ErrorInfo)` as terminating because collectibility depends on data flow and the enclosing `ErrorBehavior::Collect` scope.
