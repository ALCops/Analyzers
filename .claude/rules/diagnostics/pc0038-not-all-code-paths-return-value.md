...
| Exhaustive `case` statements do not require `else` | An enum or option case is exhaustive when every ordinal available in the current compilation is handled. Enum values are obtained through the SDK's internal `CompilationUtilities.GetEnumValues` helper, which merges visible enum-extension values. System options expose their option values through the public `IContainerSymbol.GetMembers()` contract |
