...
| Detect built-in `Error(...)` and `FieldError(...)` through `FlowTerminatingBuiltIns` | Reuses the exact `Dialog.Error`, `Table.FieldError`, and `FieldRef.FieldError` semantic classification, including invalid editor-time bindings, while excluding user-defined same-name methods. |
...
- Reports when a scope-leaving statement (`exit`, built-in `Error` or `FieldError`) is missing a blank line before it when it follows another statement in the same block.
...
- A control-flow block followed by `exit` or built-in `Error(...)` / `FieldError(...)` produces one diagnostic for the gap. The block's active "after" check owns it; otherwise the scope-leaving "before" check does.
...
- Collectible `Error(ErrorInfo)` calls are treated as scope-leaving even in an `ErrorBehavior::Collect` scope where execution can continue. This is inherited from the shared invocation classifier and requires broader flow analysis to distinguish.
