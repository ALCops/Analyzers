...
- **DataTransfer with an unresolvable `SetTables`** (none reaching the executor in the same body, or with non-literal table arguments) is not reported at all, so a genuinely missing permission there is missed. The flow walk itself accepts two imprecisions, both over-attributing rather than under-attributing: `exit` does not terminate a path, and a `SetTables` written after an executor inside a loop body does not flow back to it (single pass)
