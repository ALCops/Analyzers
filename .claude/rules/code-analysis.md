...
| CI `dotnet format --verify-no-changes` has `continue-on-error: true` | Same baseline principle. The `whitespace` (IDE0055 + CHARSET) and `style` (IDE0005) categories are clean; the umbrella command still reports a few RCS fixes at `--severity warn`. Flip to blocking once those are 0 on `main`. |
