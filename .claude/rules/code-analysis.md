...
| CI `dotnet format --verify-no-changes` runs first as `env -u GITHUB_ACTIONS dotnet format …`, and has `continue-on-error: true` | Fail fast: under `ContinuousIntegrationBuild` the test projects reference the cop DLLs by `HintPath` (`bin/$(Configuration)/<tfm>/`), which do not exist before the builds, so `dotnet format` saw 262 `CS0246` and turned every test-project using into a bogus `IDE0005`. The csproj files derive `ContinuousIntegrationBuild` from `GITHUB_ACTIONS` via a project-level assignment, which beats an env override of the property itself, and the runner refuses to override `GITHUB_*` via a step `env:` — hence unsetting it on the command line. With it masked they use `ProjectReference` and compile from source (net10.0 only — TFM-specific IDE0005 sites are still caught by the multi-TFM Build steps, where IDE0005 is `error`). `continue-on-error` is the same baseline principle: flip to blocking once RCS1075/RCS1102 are 0 on `main`. |
