...
- `IInstanceReferenceOperation` (used to detect `this`/self) is absent in the netstandard2.1 SDK; the `this.` form is guarded with `#if !NETSTANDARD2_1`, so on netstandard2.1 only the `Rec`-named instance is suppressed. The explicit `Rec.` / bare implicit-with forms still work on all targets.
