...
| Fast path: name starts with `I` and no whitespace at index 1 | No affix lookup needed for compliant names |

## Analyzer flow

1. Skip obsolete symbols; require `IInterfaceTypeSymbol`.
2. Name starts with `I` and `name[1]` is not whitespace → compliant, return.
3. `MandatoryAffixes.GetIndexAfterLeadingAffix(name, affixes)` → null ⇒ report.
4. First letter/digit of the remainder after the affix is not `I` ⇒ report.
...
**HasDiagnostic (4 cases):** NoLeadingI, WhitespaceAfterI, AffixWithoutI, AffixThenIWithWhitespace.
**NoDiagnostic (4 cases):** LeadingI, PrefixThenI, AffixThenI, SuffixAsLeadingAffixThenI.
