...
| `AcronymRegistry.cs` | Case-insensitive registry of canonical acronym casings (`LCY`, `OData`, `UoM`, `VAT`, ...). Exposes `DefaultAcronyms` (45-entry curated BC/web/data list, excludes 2-letter abbreviations and `ID`), `Default` singleton, `Create(IEnumerable<string>?)` merge factory (user entries override built-ins on identical upper-invariant key), and `TryGetCanonical(word, out canonical)`. Used by LC0098 (EventSubscriberNamingPattern); designed as shared infrastructure for future rules that render identifiers from natural-language input. |
